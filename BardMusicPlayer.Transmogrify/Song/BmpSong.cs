    ...
    public sealed class BmpSong
    {
        ...
        public MemoryStream GetDryWetSequencerMidi()
        {
            try
            {
                ...
                long firstNote = midiFile.GetTrackChunks().GetNotes().First().GetTimedNoteOnEvent().TimeAs<MetricTimeSpan>(midiFile.GetTempoMap()).TotalMicroseconds / 1000;
                ...
                Parallel.ForEach(midiFile.GetTrackChunks().Where(static x => x.GetNotes().Any() || x.Events.OfType<LyricEvent>().Any()), (originalChunk, loopState, index) =>
                {
                    ...
                    watch.Stop();
                    Debug.WriteLine("step 1 [Remove Overlap]: " + noteVelocity + ": " + watch.ElapsedMilliseconds);
                    ...
                    watch.Stop();
                    Debug.WriteLine("step 3: [Fix Spacing]" + noteVelocity + ": " + watch.ElapsedMilliseconds);
                    ...
                    watch.Stop();
                    Debug.WriteLine("step 4: [Fix Sustain]" + noteVelocity + ": " + watch.ElapsedMilliseconds);
                    #endregion
