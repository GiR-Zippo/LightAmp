...
{ 
    public class NativeFormat  {
        ...
        private MidiTrack getMidiHeader()
        {
            ...
            //end_of_track
            midiHeader.messages.Add(new MidiMessage("track_name", new string[] { "untitled" }, 0));
            midiHeader.messages.Add(new MidiMessage("text", new string[] { title }, 0));
            midiHeader.messages.Add(new MidiMessage("text", new string[] { subtitle }, 0));
            midiHeader.messages.Add(new MidiMessage("text", new string[] { artist }, 0));
            midiHeader.messages.Add(new MidiMessage("text", new string[] { album }, 0));
            midiHeader.messages.Add(new MidiMessage("text", new string[] { words }, 0));
            midiHeader.messages.Add(new MidiMessage("text", new string[] { music }, 0));
            midiHeader.messages.Add(new MidiMessage("copyright", new string[] { "Copyright 2017 by Gitaro" }, 0));
            midiHeader.messages.Add(new MidiMessage("marker", new string[] { title+" / "+artist+" - Copyright 2017 by Gitaro" }, 0));
            midiHeader.messages.Add(new MidiMessage("midi_port", new string[] { "0" }, 0));
            ...
            while (tempoIndex < tempos.Count || masterBarIndex < barMaster.Count)
            {
                ...
                if (tempoIndex == tempos.Count || tempos[tempoIndex].position >= barMaster[masterBarIndex].index) //next measure comes first
                {
                    if (!barMaster[masterBarIndex].keyBoth.Equals(oldKeySignature))
                    {
                        ...
                        midiHeader.messages.Add(new MidiMessage("key_signature", new string[] { ""+ barMaster[masterBarIndex].key, ""+ barMaster[masterBarIndex].keyType }, barMaster[masterBarIndex].index - currentIndex));
                    ...
                    if (!barMaster[masterBarIndex].time.Equals(oldTimeSignature))
                    {
                        ...
                        midiHeader.messages.Add(new MidiMessage("time_signature", new string[] { "" + barMaster[masterBarIndex].num, "" + barMaster[masterBarIndex].den, "24", "8" }, barMaster[masterBarIndex].index - currentIndex));
                        currentIndex = barMaster[masterBarIndex].index;
                ...
                else //next tempo signature comes first
                {
                    ...
                    midiHeader.messages.Add(new MidiMessage("set_tempo", new string[] { "" + _tempo }, tempos[tempoIndex].position - currentIndex));
        ...
        public List<Track> retrieveTracks(GPFile file)
        {
            ...
            foreach (GuitarPro.Track tr in file.tracks)
            {
        ...
        public void addToTremoloBarList(int index, int duration, BendEffect bend, Track myTrack)
        {
            ...
            foreach (GuitarPro.BendPoint bp in bend.points)
            {
            ...
            foreach (Measure m in track.measures)
            {
                ...
                foreach (Voice v in m.voices)
                {
                    ...
                    foreach (Beat b in v.beats)
                    {
                        ...
                        foreach (GuitarPro.Note n in b.notes)
                        {
                            Note note = new Note();
                            //Beat values
